...
public class WaterBottleView extends TextureView {
    private static final int FPS = 60;
    private static final int N_BUBBLES_PAINT = 5;
    private static final float WH_RATIO = 3.5f;
    ...
    public WaterBottleView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        setOpaque(false);

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.WaterBottleView, 0, 0);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (width * 1.25f > height)
            setMeasuredDimension(width, (int) (width * 1.25f));
        else
            setMeasuredDimension((int) (height * 0.8f), height);
    ...
    private class DrawingThread extends Thread {
        ...
        private RectF mWaterRectClipping = new RectF();
        ...
        private void updatePerc() {
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    mBubblesNumber = (int) (mPercValue * 30);

                    mPercString = (int) (mPercValue * 100f) + "%";

                    int glassThickness = (int) ((mDrawingArea.right - mDrawingArea.left) * 0.02d);
                    double cappedPerc = mPercValue * 0.9;

                    mWaterPath.reset();
                    mWaterPath.moveTo(mDrawingAreaCenter.x + mSizeOnTop / 2 - glassThickness, mDrawingArea.top + glassThickness + mSizeOnTop * ANGULAR_RATIO / 2);
                    mWaterPath.lineTo(mDrawingAreaCenter.x + mSizeOnTop / 2 - glassThickness, (mDrawingArea.top + mDrawingArea.bottom) / 48f + mSizeOnBottom * ANGULAR_RATIO / 2);
                    mWaterPath.lineTo(mDrawingAreaCenter.x + mSizeOnBottom / 2 - glassThickness, (mDrawingArea.top + mDrawingArea.bottom) / 5f + mSizeOnBottom * ANGULAR_RATIO / 2);
                    int nScalini = 6;
                    int scalino = mSizeOnBottom / 32;
                    int topPadding = 0;
                    float startScaliniY = (mDrawingArea.top + mDrawingArea.bottom) / 3.5f;
                    for (int i = 0; i < nScalini; i++) {
                        mWaterPath.lineTo(mDrawingAreaCenter.x + mSizeOnBottom / 2 - glassThickness, startScaliniY + topPadding + mSizeOnBottom * ANGULAR_RATIO / 2);
                        mWaterPath.lineTo(mDrawingAreaCenter.x + mSizeOnBottom / 2 - glassThickness - scalino, startScaliniY + topPadding + scalino + mSizeOnBottom * ANGULAR_RATIO / 2);
                        mWaterPath.lineTo(mDrawingAreaCenter.x + mSizeOnBottom / 2 - glassThickness - scalino, startScaliniY + topPadding + scalino * 2 + mSizeOnBottom * ANGULAR_RATIO / 2);
                        mWaterPath.lineTo(mDrawingAreaCenter.x + mSizeOnBottom / 2 - glassThickness, startScaliniY + topPadding + scalino * 3 + mSizeOnBottom * ANGULAR_RATIO / 2);
                        topPadding += scalino * 4;
                    }
                    mWaterPath.lineTo(mDrawingAreaCenter.x + mSizeOnBottom / 2 - glassThickness, mDrawingArea.bottom - glassThickness - mSizeOnBottom * ANGULAR_RATIO / 2);
                    mWaterPath.lineTo(mDrawingAreaCenter.x - mSizeOnBottom / 2 + glassThickness, mDrawingArea.bottom - glassThickness - mSizeOnBottom * ANGULAR_RATIO / 2);
                    topPadding = 0;
                    startScaliniY = (mDrawingArea.top + mDrawingArea.bottom) / 3.5f + scalino * 4 * (nScalini - 1);
                    for (int i = 0; i < nScalini; i++) {
                        mWaterPath.lineTo(mDrawingAreaCenter.x - mSizeOnBottom / 2 + glassThickness, startScaliniY + topPadding + scalino * 3 + mSizeOnBottom * ANGULAR_RATIO / 2);
                        mWaterPath.lineTo(mDrawingAreaCenter.x - mSizeOnBottom / 2 + glassThickness + scalino, startScaliniY + topPadding + scalino * 2 + mSizeOnBottom * ANGULAR_RATIO / 2);
                        mWaterPath.lineTo(mDrawingAreaCenter.x - mSizeOnBottom / 2 + glassThickness + scalino, startScaliniY + topPadding + scalino + mSizeOnBottom * ANGULAR_RATIO / 2);
                    ...
                    mWaterPath.lineTo(mDrawingAreaCenter.x - mSizeOnBottom / 2 + glassThickness, (mDrawingArea.top + mDrawingArea.bottom) / 5f + mSizeOnBottom * ANGULAR_RATIO / 2);
                    mWaterPath.lineTo(mDrawingAreaCenter.x - mSizeOnTop / 2 + glassThickness, (mDrawingArea.top + mDrawingArea.bottom) / 48f + mSizeOnBottom * ANGULAR_RATIO / 2);
                    mWaterPath.lineTo(mDrawingAreaCenter.x - mSizeOnTop / 2 + glassThickness, mDrawingArea.top + glassThickness + mSizeOnTop * ANGULAR_RATIO / 2);
                    mWaterPath.close();


                    mWaterRectClipping.set(mDrawingArea.left, (float) lerp(mDrawingArea.bottom - mSizeOnBottom * ANGULAR_RATIO / 2 - glassThickness, mDrawingArea.top + glassThickness, cappedPerc), mDrawingArea.right, mDrawingArea.bottom);

                    float percentedTop = (float) lerp(mDrawingArea.bottom - mSizeOnBottom * ANGULAR_RATIO / 2 - glassThickness, mDrawingArea.top + glassThickness, cappedPerc);
                    Region region = new Region();
                    region.setPath(mWaterPath, new Region(mDrawingArea.left, (int) percentedTop, mDrawingArea.right, (int) percentedTop + 1));
                    int percentedSizeOnTop = (int) (region.getBounds().right - region.getBounds().left);
                    int percentedVerticalSizeOnTop = (int) (percentedSizeOnTop * ANGULAR_RATIO);
                    int percentedSizeOnBottom = mSizeOnBottom - glassThickness * 2;

                    mMaxBubbleSize = percentedSizeOnBottom / 12;

                    mWaterTopEllipse.set(mDrawingAreaCenter.x - percentedSizeOnTop / 2, percentedTop - percentedVerticalSizeOnTop / 2, mDrawingAreaCenter.x + percentedSizeOnTop / 2, percentedTop + percentedVerticalSizeOnTop / 2);
        ...
        private void updateSize(final int w, final int h, final int pLeft, final int pTop, final int pRight, final int pBottom) {
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    mSizeOnBottom = (mDrawingArea.right - mDrawingArea.left);
                    mSizeOnTop = (int) ((mDrawingArea.right - mDrawingArea.left) * 0.3f);
                    ...
                    mGlassPath.lineTo(mDrawingAreaCenter.x + mSizeOnTop / 2, (mDrawingArea.top + mDrawingArea.bottom) / 48f + mSizeOnBottom * ANGULAR_RATIO / 2);
                    mGlassPath.lineTo(mDrawingAreaCenter.x + mSizeOnBottom / 2, (mDrawingArea.top + mDrawingArea.bottom) / 5f + mSizeOnBottom * ANGULAR_RATIO / 2);
                    ...
                    float startScaliniY = (mDrawingArea.top + mDrawingArea.bottom) / 3.5f;
                    for (int i = 0; i < nScalini; i++) {
                        mGlassPath.lineTo(mDrawingAreaCenter.x + mSizeOnBottom / 2, startScaliniY + topPadding + mSizeOnBottom * ANGULAR_RATIO / 2);
                        mGlassPath.lineTo(mDrawingAreaCenter.x + mSizeOnBottom / 2 - scalino, startScaliniY + topPadding + scalino + mSizeOnBottom * ANGULAR_RATIO / 2);
                        mGlassPath.lineTo(mDrawingAreaCenter.x + mSizeOnBottom / 2 - scalino, startScaliniY + topPadding + scalino * 2 + mSizeOnBottom * ANGULAR_RATIO / 2);
                        mGlassPath.lineTo(mDrawingAreaCenter.x + mSizeOnBottom / 2, startScaliniY + topPadding + scalino * 3 + mSizeOnBottom * ANGULAR_RATIO / 2);
                        topPadding += scalino * 4;
                    }
        ...
        private void draw(Canvas canvas) {
            if (canvas != null) {
                ...
                canvas.clipRect(mWaterRectClipping);
